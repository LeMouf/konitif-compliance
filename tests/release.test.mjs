...
const env={GITHUB_REPOSITORY:'LeMouf/konitif-compliance',GITHUB_EVENT_NAME:'push',GITHUB_REF:'refs/tags/v0.284.1'};
test('release guard accepts only the intended package, source repository and version tag', () => {
  ...
  for(const changed of [{GITHUB_REPOSITORY:'other/repo'},{GITHUB_EVENT_NAME:'pull_request'},{GITHUB_REF:'refs/heads/main'},{GITHUB_REF:'refs/tags/v0.284.2'}]) {
...
test('workflow requires opt-in, manual environment boundary and verified archive', () => {
